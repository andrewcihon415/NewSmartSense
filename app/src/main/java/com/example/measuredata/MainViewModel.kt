...
fun changeScreenColor(HR: Int, dB: Float, act: Activity) {
//add eto to the OSHA standard, running avrage make it tuneable first alert at min 10 sound, color (blink), vibrate
    when {
                HR > 120 || dB > 80 -> act.window.decorView.setBackgroundColor(Color.RED)
                HR < 80 && dB < 60 -> act.window.decorView.setBackgroundColor(Color.GREEN)
                else -> act.window.decorView.setBackgroundColor(Color.YELLOW) //add when more user feedback
            }
}
